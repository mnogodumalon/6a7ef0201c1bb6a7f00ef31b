...
export type AusleiheDialogDefaults = Omit<Ausleihe['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['ausleihe']?.['status'] ?? [],
};
...
export function AusleiheDialog({ open, onClose, onSubmit, defaultValues, recordId, handwerkerList, werkzeugeList, enablePhotoScan = true, enablePhotoLocation = true }: AusleiheDialogProps) {
  ...
  const REQUIRED_FIELDS = ['handwerker', 'werkzeug', 'ausleihdatum', 'status'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'tatsaechliches_rueckgabedatum': (
      <div key="tatsaechliches_rueckgabedatum" className="space-y-1.5">
        ...
        <DatePicker
          ...
          placeholder="Wann wurde tatsächlich zurückgegeben?"
          mode="date"
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('ausleihe', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('ausleihe', 'status', 'ausgeliehen') ?? 'Ausgeliehen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('ausleihe', 'status', 'zurueckgegeben') ?? 'Zurückgegeben'}
          </button>
        </div>
    ...
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        <Label htmlFor="bemerkungen">{fieldLabel('ausleihe', 'bemerkungen')}</Label>
        <Textarea
          id="bemerkungen"
          placeholder="Besonderheiten, Transportmittel, Schäden..."
