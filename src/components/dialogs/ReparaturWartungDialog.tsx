/**
 * ReparaturWartungDialog — pre-generated create/edit dialog for ReparaturWartung.
 *
 ...
 * ReparaturWartungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<ReparaturWartung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<ReparaturWartungDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { ReparaturWartung, Werkzeuge, Handwerker, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/ReparaturWartung';
...
/** Widened prefill type for ReparaturWartungDialog.defaultValues — see file header. */
export type ReparaturWartungDialogDefaults = Omit<ReparaturWartung['fields'], 'art' | 'status'> & {
...
interface ReparaturWartungDialogProps {
  ...
  defaultValues?: ReparaturWartungDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  art: LOOKUP_OPTIONS['reparatur_wartung']?.['art'] ?? [],
  status: LOOKUP_OPTIONS['reparatur_wartung']?.['status'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  werkzeug: APP_IDS.WERKZEUGE,
  verantwortlicher_handwerker: APP_IDS.HANDWERKER,
};
...
export function ReparaturWartungDialog({ open, onClose, onSubmit, defaultValues, recordId, werkzeugeList, handwerkerList, enablePhotoScan = true, enablePhotoLocation = true }: ReparaturWartungDialogProps) {
  ...
  const REQUIRED_FIELDS = ['werkzeug', 'art', 'beschreibung', 'startdatum', 'status'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<ReparaturWartung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'reparatur_wartung');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="verantwortlicher_handwerker" entity="Handwerker">\n${JSON.stringify(handwerkerList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["werkzeug", "verantwortlicher_handwerker"]);
        ...
        const verantwortlicher_handwerkerName = raw['verantwortlicher_handwerker'] as string | null;
        if (verantwortlicher_handwerkerName) {
          const verantwortlicher_handwerkerMatch = handwerkerList.find(r => matchName(verantwortlicher_handwerkerName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (verantwortlicher_handwerkerMatch) merged['verantwortlicher_handwerker'] = createRecordUrl(APP_IDS.HANDWERKER, verantwortlicher_handwerkerMatch.record_id);
        }
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('reparatur_wartung') })
    : t('new_entity', { entity: appLabel('reparatur_wartung') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'werkzeug': (
      <div key="werkzeug" className="space-y-1.5">
        <Label htmlFor="werkzeug">{fieldLabel('reparatur_wartung', 'werkzeug')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="werkzeug"
    ...
    'art': (
      <div key="art" className="space-y-1.5">
        <Label htmlFor="art">{fieldLabel('reparatur_wartung', 'art')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.art) === 'reparatur'}
            onClick={() => setFields(f => ({ ...f, art: (lookupKey(f.art) === 'reparatur' ? undefined : 'reparatur') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.art) === 'reparatur'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('reparatur_wartung', 'art', 'reparatur') ?? 'Reparatur'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.art) === 'wartung'}
            onClick={() => setFields(f => ({ ...f, art: (lookupKey(f.art) === 'wartung' ? undefined : 'wartung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.art) === 'wartung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('reparatur_wartung', 'art', 'wartung') ?? 'Wartung'}
          </button>
        </div>
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        <Label htmlFor="beschreibung">{fieldLabel('reparatur_wartung', 'beschreibung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Textarea
          id="beschreibung"
          placeholder="Was ist zu reparieren/warten? Mangel beschreiben..."
    ...
    'startdatum': (
      <div key="startdatum" className="space-y-1.5">
        <Label htmlFor="startdatum">{fieldLabel('reparatur_wartung', 'startdatum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="startdatum"
    ...
    'enddatum': (
      <div key="enddatum" className="space-y-1.5">
        <Label htmlFor="enddatum">{fieldLabel('reparatur_wartung', 'enddatum')}</Label>
        <DatePicker
          id="enddatum"
    ...
    'verantwortlicher_handwerker': (
      <div key="verantwortlicher_handwerker" className="space-y-1.5">
        <Label htmlFor="verantwortlicher_handwerker">{fieldLabel('reparatur_wartung', 'verantwortlicher_handwerker')}</Label>
        <Combobox
          id="verantwortlicher_handwerker"
          placeholder="Wer führt die Maßnahme durch?"
          items={handwerkerListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.vorname ?? r.record_id),
          }))}
          value={extractRecordId(fields.verantwortlicher_handwerker)}
          onChange={id => setFields(f => ({ ...f, verantwortlicher_handwerker: id ? createRecordUrl(APP_IDS.HANDWERKER, id) : undefined }))}
          onCreateNew={(q) => openCreateHandwerker("verantwortlicher_handwerker", q)}
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('reparatur_wartung', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('reparatur_wartung', 'status', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('reparatur_wartung', 'status', 'in_bearbeitung') ?? 'In Bearbeitung'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('reparatur_wartung', 'status', 'abgeschlossen') ?? 'Abgeschlossen'}
          </button>
        </div>
    ...
    'kosten': (
      <div key="kosten" className="space-y-1.5">
        <Label htmlFor="kosten">{fieldLabel('reparatur_wartung', 'kosten')}</Label>
        <Input
          id="kosten"
          type="number"
          step="any"
    ...
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        <Label htmlFor="bemerkungen">{fieldLabel('reparatur_wartung', 'bemerkungen')}</Label>
        <Textarea
          id="bemerkungen"
          placeholder="Ersatzteile, Material, Besonderheiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"werkzeug": "Werkzeug", "art": "Art der Maßnahme", "beschreibung": "Beschreibung", "startdatum": "Startdatum", "enddatum": "Enddatum", "verantwortlicher_handwerker": "Verantwortlicher Handwerker", "status": "Status", "kosten": "Kosten (€)", "bemerkungen": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>(["kosten"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"werkzeug": {"werkzeugname": "Werkzeugname", "inventarnummer": "Inventarnummer", "kategorie": "Kategorie", "hersteller": "Hersteller", "modell": "Modell", "kaufdatum": "Kaufdatum", "zustand": "Zustand", "standort": "Standort", "foto": "Foto", "bemerkungen": "Bemerkungen"}, "verantwortlicher_handwerker": {"vorname": "Vorname", "nachname": "Nachname", "personalnummer": "Personalnummer", "telefon": "Telefon", "email": "E-Mail", "abteilung": "Abteilung", "status": "Status", "bemerkungen": "Bemerkungen"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.REPARATUR_WARTUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
