...
import type { Handwerker, Werkzeuge, Ausleihe, ReparaturWartung, CreateHandwerker, CreateWerkzeuge, CreateAusleihe, CreateReparaturWartung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getReparaturWartungEntry(id: string): Promise<ReparaturWartung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as ReparaturWartung;
    return enrichLookupFields([record], 'reparatur_wartung')[0];
  }
  static async createReparaturWartungEntry(fields: CreateReparaturWartung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.REPARATUR_WARTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'reparatur_wartung') });
    return { ...data, record_id: data.id };
  }
  static async updateReparaturWartungEntry(id: string, fields: Partial<CreateReparaturWartung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.REPARATUR_WARTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'reparatur_wartung') });
    return { ...data, record_id: data.id };
  }
