...
export function WerkzeugeDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: WerkzeugeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['werkzeugname', 'inventarnummer', 'kategorie', 'zustand'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "werkzeugname": string | null, // Werkzeugname\n  "inventarnummer": string | null, // Inventarnummer\n  "kategorie": LookupValue | null, // Kategorie (select one key: "messgeraet" | "handwerkzeug" | "elektrowerkzeug" | "pruefgeraet" | "sonstiges") mapping: messgeraet=Messgerät, handwerkzeug=Handwerkzeug, elektrowerkzeug=Elektrowerkzeug, pruefgeraet=Prüfgerät, sonstiges=Sonstiges\n  "hersteller": string | null, // Hersteller\n  "modell": string | null, // Modell\n  "kaufdatum": string | null, // YYYY-MM-DD\n  "zustand": LookupValue | null, // Zustand (select one key: "gut" | "beschaedigt" | "ausser_betrieb") mapping: gut=Gut, beschaedigt=Beschädigt, ausser_betrieb=Außer Betrieb\n  "standort": string | null, // Standort\n  "bemerkungen": string | null, // Bemerkungen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'werkzeugname': (
      <div key="werkzeugname" className="space-y-1.5">
        ...
        <Input
          id="werkzeugname"
          placeholder="z. B. Bohrmaschine DeWalt"
    ...
    'kategorie': (
      <div key="kategorie" className="space-y-1.5">
        <Label htmlFor="kategorie">{fieldLabel('werkzeuge', 'kategorie')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'messgeraet'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'messgeraet' ? undefined : 'messgeraet') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'messgeraet'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('werkzeuge', 'kategorie', 'messgeraet') ?? 'Messgerät'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'handwerkzeug'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'handwerkzeug' ? undefined : 'handwerkzeug') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'handwerkzeug'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('werkzeuge', 'kategorie', 'handwerkzeug') ?? 'Handwerkzeug'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'elektrowerkzeug'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'elektrowerkzeug' ? undefined : 'elektrowerkzeug') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'elektrowerkzeug'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('werkzeuge', 'kategorie', 'elektrowerkzeug') ?? 'Elektrowerkzeug'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'pruefgeraet'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'pruefgeraet' ? undefined : 'pruefgeraet') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'pruefgeraet'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('werkzeuge', 'kategorie', 'pruefgeraet') ?? 'Prüfgerät'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('werkzeuge', 'kategorie', 'sonstiges') ?? 'Sonstiges'}
          </button>
        </div>
    ...
    'kaufdatum': (
      <div key="kaufdatum" className="space-y-1.5">
        <Label htmlFor="kaufdatum">{fieldLabel('werkzeuge', 'kaufdatum')}</Label>
        <DatePicker
          id="kaufdatum"
    ...
    'zustand': (
      <div key="zustand" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('werkzeuge', 'zustand', 'gut') ?? 'Gut'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('werkzeuge', 'zustand', 'beschaedigt') ?? 'Beschädigt'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('werkzeuge', 'zustand', 'ausser_betrieb') ?? 'Außer Betrieb'}
          </button>
        </div>
    ...
    'standort': (
      <div key="standort" className="space-y-1.5">
        ...
        <Input
          id="standort"
          placeholder="z. B. Werkstatt A, Schrank 2"
    ...
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        <Label htmlFor="bemerkungen">{fieldLabel('werkzeuge', 'bemerkungen')}</Label>
        <Textarea
          id="bemerkungen"
          placeholder="Zubehör, Besonderheiten, Wartungsintervalle..."
  ...
  const FIELD_LABELS: Record<string, string> = {"werkzeugname": "Werkzeugname", "inventarnummer": "Inventarnummer", "kategorie": "Kategorie", "hersteller": "Hersteller", "modell": "Modell", "kaufdatum": "Kaufdatum", "zustand": "Zustand", "standort": "Standort", "foto": "Foto", "bemerkungen": "Bemerkungen"};
